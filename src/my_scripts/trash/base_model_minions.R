...
for( i in 1:30 ) {
  ...
  canarito_vars = c(canarito_vars,paste0("canarito", i ))
}
...
formula_train <- paste0( "clase_binaria ~ ", campos_train )
campos_train
summary(dlearn)

to_optimize_selection <- c(variables_selected[0],"clase_binaria")
...
pdf(paste0("/Users/dfontenla/Maestria/2022C2/DMEyF/repo/exp/KA2001/img/", "pruned_tree.pdf"))
fancyRpartPlot(modelo_pruned)
dev.off()
...
TreeCVProfitPrediction(dlearn, formula_train, semillas, modelo_pruned)
...
mis_variables_ds_selected <- c(mis_variables_ds_selected, canarito_vars) 
...
impute_missing_news <- function(ds) {
  ...
  new_master_vars <- c("tenure_over_age","balance_ratio","mmaster_consumo_transacciones_ratio", # nolint
  ...
  new_visa_vars <- c("tenure_over_age","balance_ratio","mvisa_consumo_transacciones_ratio", # nolint
...
TreeCVProfitPrediction <- function(ds, formula, seeds, model) {
  gan <- c()
  auc <- c()
  ...
  for (s in seeds) {
    ...
    result <- calcular_ganancia_binaria(model, test)
    gan <- c(gan, result$profit)
    auc <- c(auc, result$auc)
    print(gan)
  }
