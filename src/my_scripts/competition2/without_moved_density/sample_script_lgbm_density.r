...
campos_modelo  <- names( modelo_last$variable.importance )
campos_buenos  <- c( campos_modelo,  setdiff( colnames(dtrain), campos_modelo ) )
campos_buenos  <-  setdiff(  campos_buenos,  c( "foto_mes","clase_ternaria","clase_binaria" ) )


dir.create( "./exp/",  showWarnings = TRUE ) 
dir.create( "./exp/density/", showWarnings = TRUE )
setwd("./exp/density/")
